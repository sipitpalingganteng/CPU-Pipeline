...
#table(
  ...
  [`pc`], [Program counter (IF), reset + enable], [sequential],
  [`imem`], [Instruction SRAM, word-indexed (Harvard)], [comb. read],
  [`control`], [decodes the ID instruction → control signals + branch type], [comb.],
  [`alu_control`], [decoded instruction → 12-bit one-hot ALU operation], [comb.],
  [`regfile`], [32 × 32 regs, 2 read / 1 write port, `$zero` masked], [sequential],
  [`sign_extend`], [16 → 32-bit sign extension of immediates], [comb.],
  [`shift_left2`], [`<<2` used for branch offsets and jump targets], [comb.],
  [`alu`], [add / sub / and / or / xor / nor / slt / sltu / shifts / lui], [comb.],
  [`forward`], [EX/MEM → ID forwarding selects], [comb.],
  [`hazard`], [load-use stall: freeze IF and ID for one cycle], [comb.],
  [`dmem`], [Data SRAM, sync write / comb. read], [sequential],
  [`mips_pipeline`], [top-level: wires the stages and pipeline registers], [structural],
)
